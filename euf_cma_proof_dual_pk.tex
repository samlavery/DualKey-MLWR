...
\begin{itemize}
    ...
    \item $\mathsf{cent}_{p_{pk}}(x) = x$ if $0 \le x < p_{pk}/2$, else $x - p_{pk}$ (maps to $[-p_{pk}/2, p_{pk}/2)$)
    \item $U^\star \subset R_{p_{pk}}^k$: commitments whose coefficients, after $\mathsf{cent}_{p_{pk}}$, take at most two distinct values across all $k n$ positions
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{if} $u \notin U^\star$: \textbf{continue} \hfill // enforce Huffman size
...
\begin{enumerate}
    ...
    \item \textbf{if} $u \notin U^\star$: \textbf{return} 0 \hfill // enforce minimal commitment
...
We treat membership in $U^\star$ as part of validity: if $\mathsf{Verify}(pk, m, \sigma) = 1$ then $u \in U^\star$.
...
\begin{itemize}
    \item $u \in U^\star$ (same low-entropy commitment filter)
...
\hline
$u$ (Huffman) & $\leq 70$ bytes & Enforced by $U^\star$ filter (at most 2 centered values) \\
...
\begin{enumerate}
    \item The adversary queried $H_2(u \| pk_1 \| m)$ before the signing query (probability $\leq q_H / |U^\star|$ per query)
    \item Two signing queries produce the same $u$ for different $(m, c)$ pairs (birthday bound)
\end{enumerate}

The total failure probability is:
\[
p_{\text{fail}} \leq \frac{q_S \cdot q_H}{|U^\star|} + \frac{q_S^2}{2 \cdot |U^\star|} \leq \frac{2 q_S q_H}{|U^\star|}
\]
\end{lemma}
...
\textbf{Pre-query collision}: The adversary guesses $u$ before it's computed. Since $u \in U^\star$ and
$
|U^\star| \leq \left(\binom{p_{pk}}{2} + p_{pk}\right) \cdot 2^{kn} \approx 2^{525}
$
for $p_{pk} = 128$ and $kn = 512$, and the adversary makes $q_H$ guesses:
\[
\Pr[\text{pre-query collision}] \leq \frac{q_H}{|U^\star|}
\]

\textbf{Inter-query collision}: Two signing queries $(m_i, c_i)$ and $(m_j, c_j)$ with $i \neq j$ produce the same $u$. By birthday bound:
\[
\Pr[\text{inter-query collision}] \leq \frac{q_S^2}{2 \cdot |U^\star|}
\]
...
&\quad + \frac{2 q_S q_H}{|U^\star|} \tag{RO programming, Lem~\ref{lem:ro-program}} \\
